...
# Endpoints:
#   /plan/today     -> daily plan with P, EV, sizing
#   /metrics        -> backtest metrics (loads JSON if present)
#   /reliability    -> calibration bins (loads JSON if present)
#   /trade/export   -> CSV export of today's plan
#   /guide/botswana -> quick-start guide for Botswana & global
#   /presets        -> watchlists
...
DEFAULT_FEES_BPS = 15           # brokerage/commissions
DEFAULT_SLIP_BPS_US = 10        # slippage, US
DEFAULT_SLIP_BPS_BSE = 60       # slippage, BSE (illiquidity)
DEFAULT_FX_BPS = 50             # FX in/out
DEFAULT_USD_PER_BWP = 1.0/13.5  # not used directly, kept for future

RISK_THRESHOLDS = {"conservative":0.60, "balanced":0.56, "aggressive":0.53}
MAX_KELLY_CAP = 0.02            # max 2% of bankroll per bet
MIN_TICKET_P = 150              # don’t suggest tiny orders
...
def _fetch_last_price_and_name(ticker: str) -> Optional[Tuple[float,str]]:
    ...
    try:
        ...
        if fast is not None and getattr(fast, "last_price", None) is not None:
...
def _heuristic_p(sent: float) -> float:
    p_raw = _sigmoid(_W0 + _W1 * max(0.0, sent)); p_cal = _CAL_A*p_raw + _CAL_B
...
def _load_model():
    ...
    if _joblib_load is None: return None
    if not os.path.exists(_MODEL_PATH): return None
    try:
...
def _build_plan(
    ...
    if preset == "Botswana":
        symbols, slip_bps, market = BSE_WATCH, DEFAULT_SLIP_BPS_BSE, "BSE"
    else:
        symbols, slip_bps, market = US_WATCH, DEFAULT_SLIP_BPS_US, "US"
    ...
    for sym in symbols:
        ...
        # Try model; else heuristic based on sentiment
    ...
    return Plan(
        ...
        cash={"suggested": len(items)==0, "reason": None if items else "No idea met probability & EV gates (holding cash)."}
    )
...
@app.get("/health")
def health():
    return {"status":"ok", "name":APP_NAME, "version":APP_VERSION, "model_loaded": bool(_load_model() is not None)}
...
async def metrics():
    ...
    with open(_METRICS_PATH, "r") as f:
        m = json.load(f)
    return Metrics(
        brier=m.get("brier_overall"), hit_rate=None, profit_factor=None, max_dd=None,
...
async def trade_export(
    ...
    for i in plan.ideas:
        rows.append([
            str(plan.asof), plan.preset, i.symbol, i.name, i.market,
            f"{i.entry}", f"{i.stop}", f"{i.take}",
            f"{i.p:.3f}", f"{i.ev*100:.2f}", str(i.size_bwp)
        ])
    buf = io.StringIO()
    for r in rows:
        buf.write(",".join(r) + "\n")
